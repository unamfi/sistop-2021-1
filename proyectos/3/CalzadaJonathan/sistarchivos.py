...
import os.path #para interactuar con el sistema operativo
...
#Funcion para agregar archivo de la computadora al sistema de archivos nuevo
def agregar(nombre):
...
def copiar(nombre):
	...
	copia=""
	while name==nombre:
		print("Escribe el nombre del nuevo archivo")
		name=input()
...
def quitar(nombre):#Recibe el nombre del archivo a eliminar
	file=open("fiUnam.img","r+")
	file.seek(1024)
	for i in range(64):
		archivoAux=file.read(15)
		if archivoAux.replace(' ','') == nombre:
			...
			file.close()
			#Limpia la memoria
			limpiar(ini,ceil(mag/1024))#El tamaño se envia en clusters
...
#Esta funcion limpia la memoria con el inicio y la magnitud.
def limpiar(ini,mag):
	file=open("fiUnam.img","r+")
	file.seek(ini*1024)
	vacio='\x00'
...
while a==True:
    ...
    print("|2. agregar archivo al Sist. de		|")
